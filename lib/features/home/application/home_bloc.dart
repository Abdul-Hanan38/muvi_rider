...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  FutureOr<void> updateSubVehicleTypes(
    ...
    data.fold(
      ...
      (success) async {
        ...
        await mergeOnTripStops(userData?.onTripRequest, data.onTripRequest);
        userData = data;
        updateFirebaseData();
      },
    );
  ...
  Future<void> showBidRideFunc(
    ...
    // ADD THESE TWO LINES:
    fmDistance = event.distance; // Store the distance
    fmDuration = double.tryParse(event.duration) ?? 0; // Store the duration
  ...
  FutureOr<void> getUserDetails(
    ...
    await data.fold(
      ...
      (success) async {
        ...
        await mergeOnTripStops(
            userData?.onTripRequest, success.data.onTripRequest);
  ...
  FutureOr<void> stopComplete(
    ...
    if (isSuccess) {
      if (userData?.onTripRequest != null && choosenCompleteStop != null) {
        final String completedId = choosenCompleteStop!;

        // 1. Instantly update local UI model state for instant rendering (super fast!)
        userData!.onTripRequest!.requestStops.firstWhere(
                (e) => e['id'].toString() == completedId)['completed_at'] =
            DateTime.now().toString();

        // 2. Perform async local persistence
        final SharedPreferences prefs = await SharedPreferences.getInstance();
        final String listKey = 'completed_stops_${userData!.onTripRequest!.id}';
        final List<String> completedList = prefs.getStringList(listKey) ?? [];
        if (!completedList.contains(completedId)) {
          completedList.add(completedId);
          await prefs.setStringList(listKey, completedList);
        }
      ...
      if (errorMessage != null) {
        emit(ShowErrorState(message: errorMessage!));
      }
      emit(HomeDataLoadingStopState());
    }
  }
  ...
  FutureOr<void> respondRequest(
    ...
    data.fold(
      ...
      (success) async {
        if (event.status == 1) {
          await setOnTripRequest(success.data);
          userData!.metaRequest = null;
          polyline.clear();
          polygons.clear();
          fmpoly.clear();
...
//  Locations
  ...
  waitingTime() async {
    ...
    waitingTimer = Timer.periodic(const Duration(seconds: 60), (timer) {
      ...
      if (userData!.onTripRequest != null &&
        ...
        updateFirebaseData();
        if (!isClosed) add(WaitingTimeEvent());
      } else {
  ...
  Future<void> mergeOnTripStops(
      OnTripData? oldOnTrip, OnTripData? newOnTrip) async {
    if (newOnTrip != null) {
      final SharedPreferences prefs = await SharedPreferences.getInstance();
      final listKey = 'completed_stops_${newOnTrip.id}';
      final List<String> localCompletedStopIds =
          prefs.getStringList(listKey) ?? [];

      final oldStops = oldOnTrip?.requestStops ?? [];
      final newStops = newOnTrip.requestStops;

      final updatedStops = List.from(newStops);
      for (var i = 0; i < updatedStops.length; i++) {
        var newStop = Map<String, dynamic>.from(updatedStops[i]);
        final String stopId = newStop['id'].toString();

        bool isCompletedLocally = localCompletedStopIds.contains(stopId);
        bool isCompletedInOldStops = false;

        if (oldOnTrip != null) {
          var oldStop = oldStops.firstWhere(
            (old) => old['id'].toString() == stopId,
            orElse: () => null,
          );
          if (oldStop == null && i < oldStops.length) {
            oldStop = oldStops[i];
          }
          if (oldStop != null && oldStop['completed_at'] != null) {
            isCompletedInOldStops = true;
          }
        }

        if (newStop['completed_at'] == null &&
            (isCompletedLocally || isCompletedInOldStops)) {
          newStop['completed_at'] = DateTime.now().toString();
          updatedStops[i] = newStop;

          if (!localCompletedStopIds.contains(stopId)) {
            localCompletedStopIds.add(stopId);
            await prefs.setStringList(listKey, localCompletedStopIds);
          }
        }
      }
      newOnTrip.requestStops = updatedStops;
    }
  }

  Future<void> setOnTripRequest(OnTripData? newOnTrip) async {
    if (newOnTrip == null && userData?.onTripRequest != null) {
      final SharedPreferences prefs = await SharedPreferences.getInstance();
      await prefs.remove('completed_stops_${userData!.onTripRequest!.id}');
    }
    await mergeOnTripStops(userData?.onTripRequest, newOnTrip);
    userData?.onTripRequest = newOnTrip;
  }
  ...
  FutureOr<void> createInstantRequest(
    ...
    await data.fold(
      ...
      (success) async {
        showGetDropAddress = false;
        await setOnTripRequest(success.ontripData);
        emit(HomeDataLoadingStopState());
      ...
      (success) {
        ...
        if (userData?.onTripRequest != null &&
          ...
          if (userData!.onTripRequest!.isTripStart == 0) {
            ...
            } else if (userData!.onTripRequest!.enableDigitalSignature == '1') {
            ...
            if (userData!.onTripRequest!.enableShipmentUnload == '1') {
              if (showImagePick == false) {
                add(ShowImagePickEvent());
              }
            } else if (userData!.onTripRequest!.enableDigitalSignature == '1') {
              ...
              if (userData!.onTripRequest!.requestStops.isNotEmpty &&
                  userData!.onTripRequest!.requestStops
                      ...
                      1) {
                add(CompleteStopEvent(id: choosenCompleteStop!));
              } else if (userData!.onTripRequest!.requestStops.isEmpty ||
                  userData!.onTripRequest!.requestStops
                          .where((e) => e['completed_at'] == null)
                          .length <=
                      1) {

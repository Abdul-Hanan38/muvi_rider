...
class AccBloc extends Bloc<AccEvent, AccState> {
  ...
  bool loadMoreNotification = false;
  ...
  Future<void> _getNotificationList(
    ...
    try {
      final page = event.pageNumber ?? 1;
      final data = await serviceLocator<AccUsecase>()
          .notificationDetails(pageNo: page.toString());
      data.fold(
        (error) {
          isLoading = false;
...
//get wallet init
  Future<void> walletInitEvent(
    ...
    if (!event.forceRefresh &&
      ...
      scrollController.removeListener(withdrawPageScrollListener);
...
//withdraw init
  Future<void> withdrawInitEvent(
    ...
    scrollController.removeListener(walletPageScrollListener);
    scrollController.removeListener(withdrawPageScrollListener);
    scrollController.addListener(withdrawPageScrollListener);
  ...
  walletPageScrollListener() {
    // Disabled auto scroll pagination in favor of manual "Load More" button.
  }
